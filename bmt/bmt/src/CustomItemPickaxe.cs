...
namespace bmt.src
{
    public class CustomItemPickaxe: Item, IContainedMeshSource
    {
        ...
        private Dictionary<int, MultiTextureMeshRef> meshrefs
        {
            get
            {
                return ObjectCacheUtil.GetOrCreate<Dictionary<int, MultiTextureMeshRef>>(this.api, "bmtpickaxesmeshrefs", () => new Dictionary<int, MultiTextureMeshRef>());
            }
        }
        ...
        public override void OnCreatedByCrafting(ItemSlot[] allInputslots, ItemSlot outputSlot, GridRecipe byRecipe)
        {
            ...
            jsonItemStack.Code = new AssetLocation("bmt:pickaxe-" + material);
            ...
            var c = jsonItemStack.Resolve(this.api.World, "sometools pickaxe", true);
        ...
        public override void GetHeldItemInfo(ItemSlot inSlot, StringBuilder dsc, IWorldAccessor world, bool withDebugInfo)
        {
            ...
            {
                dsc.Append(Lang.Get("A {0} {1} Pickaxe with a {2} handle and {3} bindings", Lang.Get("bmt:bonus-" + bonus),
            ...
            {
                dsc.Append(Lang.Get("A {0} Pickaxe with a {1} handle and {2} bindings", Lang.Get("game:material-" + material),
                                                                                            Lang.Get("bmt:handle-" + handle),
                                                                                            Lang.Get("bmt:binding-" + stri)));
                return;
            }
            ...
            {
                dsc.Append(Lang.Get("A {0} {1} Pickaxe with a {2} handle", Lang.Get("bmt:bonus-" + bonus),
                                                                                            Lang.Get("game:material-" + material),
                                                                                            Lang.Get("bmt:handle-" + handle)));
                return;
            }
            ...
            {
                dsc.Append(Lang.Get("A {0} Pickaxe with a {1} handle", Lang.Get("game:material-" + material),
                                                                                            Lang.Get("bmt:handle-" + handle)));
                return;
            }
        }
        private JsonItemStack genJstack(string json)
        {
            ...
            jsonItemStack.Resolve(this.api.World, "sometools pickaxe", true);
        ...
        {
            ContainedTextureSource cnts = new ContainedTextureSource(this.api as ICoreClientAPI, targetAtlas, new Dictionary<string, AssetLocation>(), string.Format("For render in bmt:hammer {0}", this.Code));
            cnts.Textures.Clear();
            ...
            {
                shapeName = "rtools/pickaxe";
            }
            else if (worktype.Equals("smelted"))
            {
                shapeName = "tools/pickaxe-copper";
            }
            else
            {
                shapeName = "tools/pickaxe-iron";
            }

            Shape shapeHammer = (this.api as ICoreClientAPI).Assets.TryGet("bmt:shapes/item/" + shapeName + ".json").ToObject<Shape>();
            ...
            MeshData meshPickaxe;
            (api as ICoreClientAPI).Tesselator.TesselateShape("item shape", shapeHammer, out meshPickaxe, cnts, null, 0, 0, 0, null, null);
            return meshPickaxe;
        }
