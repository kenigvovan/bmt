...
namespace bmt.src
{
    public class CustomItemHoe: ItemHoe
    {
        ...
        private Dictionary<int, MultiTextureMeshRef> meshrefs
        {
            get
            {
                return ObjectCacheUtil.GetOrCreate<Dictionary<int, MultiTextureMeshRef>>(this.api, "bmthoesmeshrefs", () => new Dictionary<int, MultiTextureMeshRef>());
            }
        }
        ...
        public override void OnCreatedByCrafting(ItemSlot[] allInputslots, ItemSlot outputSlot, GridRecipe byRecipe)
        {
            ...
            jsonItemStack.Code = new AssetLocation("bmt:hoe-" + material);
            ...
            var c = jsonItemStack.Resolve(this.api.World, "bmt hoe", true);
        ...
        public override void GetHeldItemInfo(ItemSlot inSlot, StringBuilder dsc, IWorldAccessor world, bool withDebugInfo)
        {
            ...
            {
                dsc.Append(Lang.Get("A {0} {1} Hoe with a {2} handle and {3} bindings", Lang.Get("bmt:bonus-" + bonus),
            ...
            {
                dsc.Append(Lang.Get("A {0} Hoe with a {1} handle and {2} bindings", Lang.Get("game:material-" + material),
                                                                                            Lang.Get("bmt:handle-" + handle),
                                                                                            Lang.Get("bmt:binding-" + stri)));
                return;
            }
            ...
            {
                dsc.Append(Lang.Get("A {0} {1} Hoe with a {2} handle", Lang.Get("bmt:bonus-" + bonus),
                                                                                            Lang.Get("game:material-" + material),
                                                                                            Lang.Get("bmt:handle-" + handle)));
                return;
            }
            ...
            {
                dsc.Append(Lang.Get("A {0} Hoe with a {1} handle", Lang.Get("game:material-" + material),
                                                                                            Lang.Get("bmt:handle-" + handle)));
                return;
            }
        }
        private JsonItemStack genJstack(string json)
        {
            ...
            jsonItemStack.Resolve(this.api.World, "bmt hoe", true);
        ...
        {
            ContainedTextureSource cnts = new ContainedTextureSource(this.api as ICoreClientAPI, targetAtlas, new Dictionary<string, AssetLocation>(), string.Format("For render in bmt:hoe {0}", this.Code));
            cnts.Textures.Clear();
            ...
            {
                shapeName = "rtools/hoe";
            }
            else if (worktype.Equals("smelted"))
            {
                shapeName = "tools/hoe-copper";
            }
            else
            {
                shapeName = "tools/hoe-iron";
            }

            Shape shapeHoe = (this.api as ICoreClientAPI).Assets.TryGet("bmt:shapes/item/" + shapeName + ".json").ToObject<Shape>();
            ...
            MeshData meshBlade;
            (api as ICoreClientAPI).Tesselator.TesselateShape("item shape", shapeHoe, out meshBlade, cnts, null, 0, 0, 0, null, null);
            return meshBlade;
        }
